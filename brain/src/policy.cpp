...
void Policy::Configure(const Config& cfg, Rng rng) {
    ...
    // input because a lower station (default 20 m, or a fitted cone)
...
void Policy::NoteSelfBroadcast(const Vec3& position, const Vec3& velocity,
                               float now) {
    if (cfg_.drone_id >= kMaxFleet) return;
    heard_[cfg_.drone_id] = now;
    heard_at_[cfg_.drone_id] = position;
    heard_vel_[cfg_.drone_id] = velocity;
}
...
void Policy::Decide(TrackStore& store, const swarm::Observation& obs) {
    ...
    if (!Intercepting(mode_)) {
        ...
        if (candidate) {
            ...
            std::snprintf(last_log_, sizeof(last_log_),
                          ...
                          early ? " early" : "");
            // Who the ring would have sent, and the two solutions, so a
            // handoff is legible in the viewer rather than inferred from a
            // drone leaving a slot it was not facing (D66).
            const uint32_t inc = FacingOwner(*candidate, store, obs);
            if (inc != cfg_.drone_id) {
                const size_t used = std::strlen(last_log_);
                if (used + 40 < sizeof(last_log_)) {
                    std::snprintf(last_log_ + used, sizeof(last_log_) - used,
                                  " hand=%u s=%.2f si=%.2f", inc,
                                  ScoreFor(cfg_.drone_id, *candidate, obs),
                                  ScoreFor(inc, *candidate, obs));
                }
            }
        }
    }
    ...
    if (!Intercepting(mode_)) {
        ...
        watch_ = nullptr;
        leashed_ = false;
        goal_vel_ = Vec3();   // intercepting: the goal is a meeting point
    }
}
...
float StationBearing(uint32_t id, uint32_t fleet_size, uint32_t self_id,
    ...
    const uint32_t rank = LiveRank(id, n, self_id, heard, heard_at, self_pos,
                                   comm_radius, now, confirmed_dead);
    // Shared function of sim time, so every drone places every station
    // identically without negotiating (D66).
    return 2.0f * kPi * static_cast<float>(rank) / static_cast<float>(live)
           + kOrbitRate * now;
}

Vec3 StationVelocityAt(float bearing, float radius) {
    // d/dt of StationAt with bearing advancing at kOrbitRate: tangential, wR.
...
float InterceptScore(const Vec3& position, const Vec3& velocity,
                     ...
                     const Config& cfg) {
    const flight::Course c = flight::SolveCollisionCourse(
        position, velocity, target_position, target_velocity, cfg);
...
float Policy::ScoreFor(uint32_t id, const Track& t,
    ...
    // Every candidate from the same source, self included, or the fleet does
    // not agree on the answer (D66).
...
uint32_t Policy::FacingOwner(const Track& t, const TrackStore& store,
                             const swarm::Observation& obs) const {
...
bool Policy::OwnsInbound(const Track& t, const TrackStore& store,
                         const swarm::Observation& obs) const {
    uint32_t owner = FacingOwner(t, store, obs);
    // The facing drone is the incumbent. Hand over only to one with a
    // decisively better solution -- already moving the right way, or able to
    // spend its larger vertical authority -- rather than merely standing
    // nearer (D66). Every drone scores every peer from the same broadcast
    // poses, so this needs nothing extra on the wire and they all agree.
    if (kHandoff) {
...
Vec3 Policy::PicketGoal(const TrackStore& store, const swarm::Observation& obs) {
    ...
    watch_ = nullptr;
    Vec3 goal = slot;
    // Holding an orbiting ring means matching its motion. Zeroed below if the
    // goal stops being the station.
...
void Policy::Compose(Outbox<24>& outbox, TrackStore& store,
    ...
    if (now - last_heartbeat_ >= 1.0f / kHeartbeatHz) {
        ...
        if (w.ok() && outbox.Push(buffer, w.size(), kPrioHeartbeat, now)) {
            last_heartbeat_ = now;
            // Peers will score us from exactly these bytes; score ourselves
            // from them too so everyone computes the same owner (D66).
            NoteSelfBroadcast(m.position, m.velocity, now);
        }
    }
