...
| flight.h:ProNav | `navigation_gain` | **7.0** (was 3.5) | **MEASURED** | Same grid: N=5 → +39.1, N=6 → +99.4, N=7 → +99.1, N=9 → +38.3, N=12 → +37.5 | Yes. The lead intercept arrives with far more closing speed than 3.5 was tuned for; 3.5 missed by 1–3 m against a 1 m kill radius. 6 and 7 tie, so this sits on a plateau. |
...
| policy.cpp:OwnsInbound | ring-slot owner | facing slot, then first live clockwise | **DERIVED** | Same angle as `RingSlot`. Heartbeat liveness (D11/D15). | Unique always. D11's ±1 both-neighbours was the two-on-one. D19 does not change this — only the stations. |
| policy.cpp:RingAlive | nearby-death keep | `comm_radius − cruise·1.5 s − 10 m` | **DERIVED** | A neighbour cannot leave radio in three missed beats. Opposite-side drones can. D19. | s1 4/6 when silence alone dropped the far side off the ring. |
