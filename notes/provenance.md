...
| policy.cpp:44 | `ring_radius_` | radio `asset + comm*0.625` then **closed-cover shrink** (D58); **86 m** on s1 | **MEASURED** cap + **DERIVED** shrink | Radio F swept with D21. Cover: unique-owner Voronoi-edge inbound at picket elevation, same first-sight / Reach as the kill-envelope bracelet. Shrink only. | On s1 (16, sense 60, H=25-30) cover does not bind. It binds on a 6-picket radio ring and on sense 35 m. Growing to buy time is the D21 F0.75 collapse. |
...
| policy.cpp:45 | `ring_altitude_` | **25 m** until a cone, then `h0 + slope · ring_r` | **DERIVED** (D52 / D58) | Floor 6 m, **cap 30 m** (friendly spawn). Default is 5 m below spawn so the bracelet sits in the inbound band hostiles actually use. D54 rejected 20 m and 12 m; this is not those. Not `spawn.enemy_altitude`. | Default 25 m is a new measurement (D58). Cap stays spawn. |
