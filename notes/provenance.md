...
| policy.cpp:44 | `ring_radius_` | `asset_radius + comm_radius·0.625` = **86 m** on s1 | **MEASURED** | Swept F ∈ {0.5, 0.625, 0.75, 0.875, 1.0} against gap bisection over 8 fixed scenarios and 20 fresh `--new-token` ids (D21) | Yes, but only jointly with D21. The fraction is not independently meaningful: F0.75 without gap closing is a wash (mean −30.9 vs −30.5); with it, F0.625 improves all 8 fixed scenarios and lifts the fresh-id mean −52.5 → −20.8. Above ~0.75 tier-2 collapses — a leaker past the ring cannot be run down, since a hostile shares our lateral limit. The neighbour chord at 86 m / 16 drones is **33.6 m**, still inside `sense_radius` 60 and `comm_radius` 90. |
...
| flight.cpp:BarrierAim | lead in front of meeting | `8 · kill_radius` | **DERIVED** | D39. Meeting plus eight radii along their track. 4·kill: this id 2.5 m → 2.0 m (kill 1.85), still a miss. 8·kill: ram at 38.4. | *Too large:* sit far out, they evade around; 8-fixed mean dipped 163.9 → 161.2. *Too small:* the 2.5 m glancing pass. On-bearing intercepts still charge along the LOS. |
| policy.cpp:45 | `ring_altitude_` | 30.0 m until a cone, then `h0 + slope · ring_r` | **DERIVED** (D52) | Linear inbound `h = h0 + slope · r` from local diving/Hostile tracks and hopped `Ray` messages. Floor 6 m, cap 30 m (friendly spawn). Not `spawn.enemy_altitude` — that is unpublished. | Yes as a measured cone, with the D32 caveat that the dive is generator behaviour. |
| policy.cpp:slot | slot-reached radius | 8.0 m | **GUESS** | none | Low-risk. |
| policy.cpp:OwnsInbound | ring-slot owner | facing slot, then first live clockwise | **DERIVED** | Same angle as `RingSlot`. Heartbeat liveness (D11/D15). Bisector deadband D38. | Unique always. D11's ±1 both-neighbours was the two-on-one. D19 does not change this — only the stations. |
...
| policy.cpp:RingAlive | nearby-death keep | in radio of last pose, **latched** until heartbeat (D37) | **DERIVED** | D19/D30: current range. D37: leaving the bubble does not resurrect. Opposite-side / interceptor silence (never confirmed nearby) stays live. | s1 4/6 when silence alone dropped the far side. Option 2 (original neighbour ⇒ dead) re-broke s2. Option 3 (last-contact only) re-broke x2-b. |
