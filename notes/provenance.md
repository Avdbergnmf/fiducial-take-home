...
| policy.cpp:44 | `ring_radius_` | radio `asset + comm*0.625` then **closed-cover shrink** (D58 / D70); **86 m** on s1 | **MEASURED** cap + **DERIVED** shrink | Radio F swept with D21. Cover: unique-owner Voronoi-edge inbound at picket elevation, leftover Reach ≥ `kCoverSlack` (5 m), else leftover ≥ 0. Same first-sight / Reach as the kill-envelope bracelet. Shrink only; may pull to the asset cylinder (not a collision). | On s1 (16, sense 60, H=25-30) leftover is ~26 m — slack does not bind. It binds on a 6-picket radio ring and on sense 35 m. Growing to buy time is the D21 F0.75 collapse. |
| policy.h | `kCoverSlack` | **5 m** leftover Reach | **MEASURED** (D70) | D58's largest-closed R is leftover ≈ 0 (knife-edge belt). 5 m does not move the identity 8. Hard canary 2/3 −113.4 → 3/3 +45.5. If 5 m is impossible, fall back to leftover 0 so tight sense does not keep an open radio ring. | Too high: tight layouts give up and sit at the caps. Too low: the overlay sits on a knife-edge whenever cover binds. |
...
| policy.cpp:45 | `ring_altitude_` | **25 m** until a cone, then `h0 + slope · ring_r`, **floor `PicketFloorAltitude`** | **DERIVED** (D52 / D58 / D68) | Cap 30 m (friendly spawn). Floor is `max(6 m, R·tan(10°), ½·Reach(sense/maxv, lat))` so the kill-envelope bracelet is not the horizon cell and half the Cover pancake is not buried. Default is 5 m below spawn. Not `spawn.enemy_altitude`. | Default 25 m is D58. The 6 m ray floor buried the belt (D68). |
