...
| belief.cpp:83 | heartbeat / hop-0 report range gate | `3σ + 2 m` | **DERIVED** | `f.range` / `f.range_sigma` are receiver measurements (`swarm_abi.h`). Hop-0 TrackReport carries author pose (D76) and uses the same residual. | The 2 m covers PosQ (0.125 m) and a few ticks of latency. Sender coasted by frame age, cap 2.5 s. A far-side replay misses by tens of metres. Hopped frames have no range to the author. Last-heartbeat lookup failed s3 (pose poison). |
...
| policy.cpp | `OtherConnectsFirst` | mate `InterceptScore` is a hit we do not take | **DERIVED** (D75) | Parked: clearly first by `kHandoffMargin`. Chasing: hit first / we miss / range-id tie. Replaces D71's 12 m ring-gap. | Catchable-from-rest is not this. A miss of our own does not yield to a parked picket. |
| policy.cpp | `ReallyChasing` | `FlyingAt` and speed > ωR + **3 m/s** | **DERIVED** (D75) | Fat-ring orbit is ωR ≈ 5 m/s at R = 84, which is `kChasingToward`. x2-b's interceptor aborted at 17 m to an orbiting picket. Yield still uses `FlyingAt`. | Too tight: a slow turn into the ram is not a chaser. Too loose: station-keeping steals the inbound. |
